# 👋 Hi, I'm Shubham Raut

🎯 **Data Analyst @ InLighnX Global Pvt. Ltd.**  
Master’s in Statistics • Strong foundation in Mathematics • Experienced in EDA, data cleaning, machine learning, SQL, dashboards, and predictive analytics.

I enjoy transforming complex datasets into clear, meaningful insights that enable smarter decisions.

---

## 🔧 Tech Stack & Tools
![Python](https://img.shields.io/badge/Python-blue?logo=python&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?logo=r&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?logo=mysql&logoColor=white)
![Power BI](https://img.shields.io/badge/PowerBI-F2C811?logo=powerbi&logoColor=black)
![Excel](https://img.shields.io/badge/Excel-217346?logo=microsoft-excel&logoColor=white)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-orange)
![Statistics](https://img.shields.io/badge/Statistics-grey)

---

## 📂 Featured Projects
🔹 [Credit Card Fraud Detection](https://github.com/shubhamraut0209/Data-Analyst-Internship-Projects/tree/main/Credit-Card-Fraud-Detection)
📉 Built ML models (Logistic Regression, XGBoost, Random Forest) to classify fraud on imbalanced datasets using SMOTE and ROC-AUC evaluation.  

🔹 [Airbnb Price Prediction](https://github.com/shubhamraut0209/Data-Analyst-Internship-Projects/tree/main/Airbnb-Price-Prediction-NYC)
🏡 Developed regression models to estimate prices using feature engineering and EDA.  

🔹 [Voice-Activated AI Chatbot](https://github.com/shubhamraut0209/Data-Analyst-Internship-Projects/tree/main/Voice-Activated-AI-Chatbot)  
🎤 Automated tasks using speech recognition, NLP, and Python APIs.

🔹 [Market Basket Analysis – Online Retail II](https://github.com/shubhamraut0209/Personal-Projects-Portfolio)  
🛒 Generated association rules to uncover purchasing behavior.

🔹 [TATA: Data Visualization Job Simulation](https://github.com/shubhamraut0209/TATA-Data-Visualization-Project)  
📊 Designed business dashboards using Power BI and Tableau.

🔹 [JPMorgan Quantitative Research Simulation](https://github.com/shubhamraut0209/JPMorgan-Quantitative-Research-Project)  
📉 Implemented predictive modeling for commodity storage & credit risk use-cases.


---

## 📌 Currently Working On
- Advanced ML projects for real-world datasets  
- Improving analytics workflows with Python & SQL  
- Building new dashboards and model-driven insights  
- Expanding portfolio with end-to-end data projects 

---

## 🚀 Let's Connect!
🔗 [LinkedIn](https://www.linkedin.com/in/shubham-raut-986bb1227)  
📫 Email: shubhamraut123.com@gmail.com
🌐 [Portfolio](https://github.com/shubhamraut0209)

---

💡 *“Every dataset holds a story — my job is to uncover it with clarity, logic, and impact.”*

